...
void notificaFilaPorCPF(Fila *fila, char *cpf) {
    for (int i = 0; i < fila->count; i++) {
        Ticket *ticket = fila->buffer[i];

        char *cpfTicket = getCPFSolicitanteTicket(ticket);

        if (strcmp(cpfTicket, cpf) == 0) {
            notificaTicket(ticket);
        }
    }
}